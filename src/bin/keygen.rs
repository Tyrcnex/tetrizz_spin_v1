...
fn main() {
    ...
    if !parsed.game.can_spawn(parsed.queue[0]) {
        println!("{}", serde_json::to_string(&OutObj {
            ...
            desired_location: PieceLocation { piece: parsed.queue[0], rotation: Rotation::Up, spun: false, x: 4, y: 21, possible_line_clear: false}
        }).unwrap());
        return;
    }

    let found_move = search(&parsed.game, parsed.queue.clone(), &eval, parsed.beam_depth, parsed.beam_width);
    ...
    println!("{}", serde_json::to_string(&OutObj {
        keys, desired_location: found_move
    }).unwrap())
}